// xdp_fw.c — AF_XDP zero-copy SNAT/NAPT 用户态转发器
//
// 工作模式：
//   LAN (eth0) <-- AF_XDP ZC --> [本程序] <-- AF_XDP ZC --> WAN (eth1)
//   BPF xdp_fw_kern.o 驱动层只 REDIRECT IPv4 TCP/UDP 到用户态；
...
//
// 编译（OpenWrt SDK / 目标机）：
//   musl-gcc -O2 -Wall -o xdp_fw xdp_fw.c -lxdp -lbpf -lelf -lz -lpthread
// 或 OpenWrt package Makefile（见同目录 Makefile）。
//
// 运行：
//   xdp_fw --wan eth1 --lan eth0 --wan-ip 192.168.1.220 --lan-cidr 192.168.1.0/24
//
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "\n"
        "  发送 SIGUSR1 导出连接表到 /run/xdp-fw.conns；\n"
        "  每秒写统计到 /run/xdp-fw.stats。\n"
        "\n"
        "示例:\n"
...
static void write_stats_file(void) {
    FILE *f = fopen("/run/xdp-fw.stats.tmp", "w");
    if (!f) return;
    ...
    fclose(f);
    rename("/run/xdp-fw.stats.tmp", "/run/xdp-fw.stats");
}

static void dump_conns_file(void) {
    FILE *f = fopen("/run/xdp-fw.conns.tmp", "w");
    if (!f) return;
    ...
    fclose(f);
    rename("/run/xdp-fw.conns.tmp", "/run/xdp-fw.conns");
    LOGI("已 dump 连接表到 /run/xdp-fw.conns");
}
...
int main(int argc, char **argv) {
    ...
    struct bpf_program *wan_prog = bpf_object__find_program_by_name(wan_obj, "xdp_fw_kern");
    struct bpf_program *lan_prog = bpf_object__find_program_by_name(lan_obj, "xdp_fw_kern");
    if (!wan_prog || !lan_prog) { LOGE("BPF 对象中找不到 xdp_fw_kern"); return 1; }
    ...
    /* 清理 stats/conns 文件 */
    unlink("/run/xdp-fw.stats");
    unlink("/run/xdp-fw.conns");
