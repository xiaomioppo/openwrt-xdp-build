 ...
 *   - 高亮冲突：xdp-nat 配置的 wan_if/lan_if 上挂着非 nat-redirect 程序时给红字提示
 *   - 服务的启停由 procd autostart / CLI / 后端自动判断（与该页面解耦）
 */

function runCtl(args) {
    return fs.exec('/usr/sbin/xdp-nat-ctl', args).catch(function(e){
...
function classifyProg(prog) {
    ...
    if (/xdp_nat_redirect/i.test(prog) || /xdp_nat$/.test(prog))
        return { kind:'nat', label:'xdp-nat redirect', color:'#16a34a' };
...
return view.extend({
    load: function() {
        ...
        return Promise.all([
            p(fs.read('/run/xdp-nat.stats'), ''),
            runCtl(['conns']),
            uci.load('xdp-nat'),
            runLoader(['status']),
            // BusyBox 的 pgrep 不支持 -c；直接打印 PID 列表，再在 JS 里数行
            p(fs.exec('/usr/bin/pgrep', ['xdp_nat']), {stdout:''}),
            p(fs.exec('/etc/init.d/xdp-nat', ['enabled']), {code:1}),
    ...
    render: function(data) {
        ...
        var wan_if = uci.get('xdp-nat', 'main', 'wan_if') || 'eth1';
        var lan_if = uci.get('xdp-nat', 'main', 'lan_if') || 'eth0';
        ...
        var root = E('div', {'class':'cbi-map'}, [
            E('h2', {}, _('XDP NAT (AF_XDP zero-copy)')),
            E('div', {'class':'cbi-map-descr'}, [
                _('用户态 NAT 数据面。本页配置规则、查看状态，并提供"自动启用"按钮。'),
                E('br'),
                _('每个网卡同时只能挂一个 XDP 程序，若要手动切换到 XDP 加速（包过滤），请到 '),
                E('a', {'href':'?'+encodeURIComponent('admin/network/xdp')}, _('网络 → XDP 加速')),
                _(' 页面。')
            ])
        ]);
        ...
        if (kernelNoXsk) {
            root.appendChild(E('div', {
                ...
                E('ol', {'style':'margin:0 0 6px 24px;font-size:13px;line-height:1.8'}, [
                    ...
                    E('li', {}, [
                        E('strong', {}, _('B. 改方案')),
                        _('：不用 AF_XDP，改用 TC eBPF + 内核常规 socket 做 NAT（性能没 zero-copy 高但无需改内核）。这等于重写 xdp-nat。')
                    ])
                ]),
        ...
        var snapshot = E('div', {'class':'cbi-section'}, [
            ...
            E('p', {'class':'cbi-value-description'}, [
                ...
                _('，开机自动）。WAN/LAN 接口若被 '),
                E('code',{},'xdp_main'),
                _(' 占用，xdp-nat 无法接管，请先点"禁用"释放该接口。')
            ])
        ]);
        ...
        var m, s, o;
        m = new form.Map('xdp-nat', null, null);

        s = m.section(form.NamedSection, 'main', 'xdp-nat', _('主配置（选择入/出网口）'));
        s.anonymous = false;
        ...
        bottom.push(E('div', {'class':'cbi-section'}, [
            ...
            E('p', {'class':'cbi-value-description'},
                _('先在上方选好 WAN / LAN 接口与 IP、CIDR，点击"保存并应用"后，再点击下方按钮即可自动释放接口上的 XDP 占用并启动 xdp-nat 服务。')),
            ...
            E('p', {'class':'cbi-value-description', 'style':'margin-top:10px;color:#6b7280;font-size:12px;line-height:1.6'}, [
                ...
                E('code',{},'/etc/init.d/xdp-nat enable'), ' → ',
                E('code',{},'restart'), '。',
                E('br'),
                _('若 WAN/LAN 当前被 xdp_main（XDP 加速）占用，这一步会先把它切到 off，由 xdp-nat 接管。')
            ])
        ]));
    ...
    onAutoEnable: function() {
        ...
        var wan_if   = uci.get('xdp-nat', 'main', 'wan_if');
        var lan_if   = uci.get('xdp-nat', 'main', 'lan_if');
        var wan_ip   = uci.get('xdp-nat', 'main', 'wan_ip');
        var lan_cidr = uci.get('xdp-nat', 'main', 'lan_cidr');
        ...
        return fs.exec('/etc/init.d/xdp-nat', ['stop']).catch(function(){return null;}).then(function(){
            ...
            return fs.exec('/etc/init.d/xdp-nat', ['enable']);
        }).then(function(){
            return fs.exec('/etc/init.d/xdp-nat', ['start']);
        }).then(function(){
            /* 步骤 3：给守护进程 2 秒时间稳定（或 crash） */
            return sleep(2000);
        }).then(function(){
            /* 步骤 4：验证是不是真活着 */
            return fs.exec('/usr/bin/pgrep', ['xdp_nat']).catch(function(){return {stdout:''};});
        }).then(function(r){
            ...
            return fs.exec('/sbin/logread', []).catch(function(){return {stdout:''};}).then(function(lr){
                ...
                ui.addNotification(null, E('div', [
                    E('p', {}, E('strong', {'style':'color:#dc2626'},
                        '❌ ' + _('XDP NAT 启动后 2 秒内 crash（未检测到 xdp_nat 进程）'))),
                    E('p', {'style':'font-size:13px;margin:6px 0'},
    ...
    onStopNat: function() {
        return fs.exec('/etc/init.d/xdp-nat', ['stop']).then(function(r){
            var ok  = (r && r.code === 0);
